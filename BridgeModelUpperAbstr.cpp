...
BridgeModelUpperAbstr::State::State(const HANDS_TYPE &hands, const LEFTS_TYPE &lefts, char next,
...
BridgeModelUpperAbstr::State::State() = default;

bool BridgeModelUpperAbstr::State::operator<(const BridgeModelUpperAbstr::State &rhs) const {
    if (hands < rhs.hands)
        return true;
    if (rhs.hands < hands)
        return false;
...
BridgeModelUpperAbstr::BridgeModelUpperAbstr(int noCardsAvailable, int noEndCards,
                                             BridgeModelUpperAbstr::State firstState,
...
void BridgeModelUpperAbstr::createAtlModel() {
...
void BridgeModelUpperAbstr::generateAvailableCards() {
...
void BridgeModelUpperAbstr::generateBeginningStates() {
...
void BridgeModelUpperAbstr::generateRestOfModel() {
    ...
    while (!this->states.empty()) {
        ...
        this->states.pop();
        ++currentStateNumber;
        std::map<int, std::set<std::pair<State, std::vector<std::string> > > > transitions;
...
//            this->printHands(contentState);
            if (contentState.next == contentState.beginning && contentState.clock == 0) {
                ...
                for (int i = 0; i < contentState.hands[contentState.next].size(); ++i) {
                    ...
                    transitions[newStateNumber].insert(std::make_pair(contentState, actions));
...
//                    this->model.addTransition(currentStateNumber, newStateNumber, actions);
        ...
        unsigned long noContentStates = this->abstractStateContent[state].size();
        this->abstractStateContent[state].clear();

        for (auto el : transitions) {
            int newStateNumber = el.first;
            if (el.second.size() >= noContentStates) {
                std::vector<std::string> actions = (el.second.begin())->second;
                this->model.addTransition(currentStateNumber, newStateNumber, actions);
            } else {
...
int BridgeModelUpperAbstr::addState(BridgeModelUpperAbstr::State &state) {
...
int BridgeModelUpperAbstr::getStateNumber(BridgeModelUpperAbstr::State &state) {
...
void BridgeModelUpperAbstr::addToEpistemicDictionary(BridgeModelUpperAbstr::State &state, int newStateNumber) {
...
BridgeModelUpperAbstr::State BridgeModelUpperAbstr::getEpistemicState(BridgeModelUpperAbstr::State &state) {
...
void BridgeModelUpperAbstr::prepareEpistemicRelation() {
    this->model.finishEpistemicClasses(0);
//    this->model.clearTransitions();
}

BridgeModelUpperAbstr::State
BridgeModelUpperAbstr::newStateAfterPlay(BridgeModelUpperAbstr::State &state, char cardIndex) {
...
char BridgeModelUpperAbstr::newSuit(BridgeModelUpperAbstr::State &state, CARD_TYPE &card) {
...
BOARD_TYPE BridgeModelUpperAbstr::newBoardAfterPlay(BridgeModelUpperAbstr::State &state, CARD_TYPE card) {
    BOARD_TYPE newBoard = state.board;
    newBoard[state.next] = card;
    return newBoard;
}

int BridgeModelUpperAbstr::countRemainingCards(BridgeModelUpperAbstr::State &state) {
...
char BridgeModelUpperAbstr::getWinner(char &beginning, BOARD_TYPE &board) {
...
std::vector<char> BridgeModelUpperAbstr::getWinners(char &beginning, BOARD_TYPE &board) {
    std::vector<CARD_TYPE> cards;
    cards.reserve(4);
    for (int i = 0; i < 4; ++i) {
        cards.push_back(board[(beginning + i) % 4]);
...
HAND_TYPE BridgeModelUpperAbstr::keepValuesInList(HAND_TYPE &list, CARD_TYPE value) {
...
std::vector<std::string> BridgeModelUpperAbstr::generateReadableCardsArray() {
...
void BridgeModelUpperAbstr::generateCardsDictionary() {
...
std::vector<std::vector<std::string> > BridgeModelUpperAbstr::handsToReadableHands(HANDS_TYPE &hands) {
...
HANDS_TYPE BridgeModelUpperAbstr::generateRandomHands(int &noCardsAvailable, int &noCardsInHand) {
    ...
    this->generatedFirstState = hands;

    return hands;
}

AtlModel &BridgeModelUpperAbstr::getModel() {
    return model;
}

void BridgeModelUpperAbstr::printHands(BridgeModelUpperAbstr::State &state) {
...
unsigned long BridgeModelUpperAbstr::getBeginningStatesCount() const {
    return beginningStatesCount;
}

void BridgeModelUpperAbstr::clear() {
    this->cardsAvailable.clear();
    this->epistemicStateClassNumber.clear();
    this->nextLevelStates.clear();
}

bool BridgeModelUpperAbstr::isWinningState(State &state) {
    return (state.lefts[0] > this->noEndCards / 2 && state.lefts[0] + state.lefts[1] == this->noEndCards);
}

const std::set<int> &BridgeModelUpperAbstr::getWinningStates() const {
    return winningStates;
}

void BridgeModelUpperAbstr::abstractState(BridgeModelUpperAbstr::State &state) {
...
CARD_TYPE BridgeModelUpperAbstr::abstractCard(CARD_TYPE card) {
